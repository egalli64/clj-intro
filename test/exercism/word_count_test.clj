...
(ns exercism.word-count-test
  ...
  (:require [exercism.word-count :refer [word-count]]))

(deftest word-count_test_1
  (testing "count one word"
    (is (= {"word" 1} (word-count "word")))))

(deftest word-count_test_2
  (testing "count one of each word"
    (is (= {"one" 1, "of" 1, "each" 1} (word-count "one of each")))))
...
(deftest word-count_test_4
  (testing "handles cramped lists"
    (is (= {"one" 1, "two" 1, "three" 1} (word-count "one,two,three")))))

(deftest word-count_test_5
  (testing "handles expanded lists"
    (is (= {"one" 1, "two" 1, "three" 1} (word-count "one,\ntwo,\nthree")))))
