...
(ns exercism.etl-test
  (:require [clojure.test :refer :all])
  (:require [exercism.etl :refer [transform]]))

(deftest transform_test_1
  (testing "single letter" (is (= {"a" 1} (transform {1 ["A"]})))))

(deftest transform_test_2
  (testing "single score with multiple letters"
    (is (= {"a" 1 "e" 1 "i" 1 "o" 1 "u" 1} (transform {1 ["A" "E" "I" "O" "U"]})))))

(deftest transform_test_3
  (testing "multiple scores with multiple letters"
    (is (= {"a" 1 "d" 2 "e" 1  "g" 2} (transform {1 ["A" "E"], 2 ["D" "G"]})))))
